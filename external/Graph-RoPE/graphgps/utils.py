...
def dirichlet_energy(x, edge_index, batch=None):
    """LoG representation-smoothing diagnostic (logging only)."""
    with torch.no_grad():
        src, dst = edge_index
        deg = degree(src, num_nodes=x.shape[0])
        normalized = x / torch.sqrt(deg + 1.0).view(-1, 1)
        energy = torch.norm(
            normalized[src] - normalized[dst], dim=1, p=2
        ) ** 2.0
        if batch is not None:
            energy = scatter(
                energy, batch[dst], dim_size=x.shape[0], reduce='mean'
            )
        else:
            energy = energy.mean()
        energy *= 0.5
...
def mean_average_distance(x, edge_index, batch):
    """LoG mean adjacent cosine-distance diagnostic (logging only)."""
    with torch.no_grad():
        src, dst = edge_index
        distance = 1 - torch.cosine_similarity(x[src], x[dst], dim=1)
        distance = scatter(
            distance, dst, dim_size=x.shape[0], reduce='sum'
        )
        distance = scatter(
            distance, batch, dim_size=x.shape[0], reduce='mean'
        )
    return float(distance.mean().detach().cpu())


def mean_norm(x):
    """LoG mean node-embedding norm diagnostic (logging only)."""
    with torch.no_grad():
