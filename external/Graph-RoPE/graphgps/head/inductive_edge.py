...
class GNNInductiveEdgeHead(nn.Module):
    ...
    def compute_mrr(self, batch):
        ...
        for data in batch.to_data_list():
            ...
            if num_pos_edges > 0:
                # Raw MRR: rank each true tail against every other node.
                neg_mask = torch.ones(
                    [num_pos_edges, data.num_nodes],
                    dtype=torch.bool,
                    device=pred.device,
                )
                neg_mask[
                    torch.arange(num_pos_edges, device=pred.device),
                    pos_edge_index[1],
                ] = False
                pred_neg = pred[pos_edge_index[0]][neg_mask].view(num_pos_edges, -1)
                # print(pred_neg, pred_neg.shape)
                mrr_list = self._eval_mrr(
                    pred_pos, pred_neg, 'torch', suffix=''
                )

                # Filtered MRR removes other true contact tails for the same
                # head from the candidate set.
                pred_masked = pred.clone()
                pred_masked[pos_edge_index[0], pos_edge_index[1]] -= float('inf')
                pred_neg = pred_masked[pos_edge_index[0]]
                mrr_list.update(self._eval_mrr(
                    pred_pos, pred_neg, 'torch', suffix='_filt'
                ))

                # Extended filtered MRR additionally removes self-loops.
                nodes = torch.arange(data.num_nodes, device=pred.device)
                pred_masked[nodes, nodes] -= float('inf')
                pred_neg = pred_masked[pos_edge_index[0]]
                mrr_list.update(self._eval_mrr(
                    pred_pos, pred_neg, 'torch', suffix='_filt_self'
                ))
            else:
                # Return empty stats.
                mrr_list = self._eval_mrr(
                    pred_pos, pred_pos, 'torch', suffix=''
                )
