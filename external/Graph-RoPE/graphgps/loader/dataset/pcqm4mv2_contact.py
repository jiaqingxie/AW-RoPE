...
def cxsmiles_to_mol_with_contact(cxsmiles):
    # RDKit is required only when converting raw CXSMILES during dataset
    # processing.  Loading the already prepared official PyG artifact for
    # training must not import this optional preprocessing dependency.
...
class PygPCQM4Mv2ContactDataset(InMemoryDataset):
    ...
    def download(self):
        # AW-ROPE prepares the official LRGB processed asset before entering
        # the offline GPU allocation. PyG checks raw paths before processed
        # paths, so without this guard it would still contact the historical
        # TSV URL even though no processing is needed.
        prepared = osp.join(
            self.folder, 'processed', 'geometric_data_processed.pt'
        )
        if osp.isfile(prepared) and osp.getsize(prepared) > 0:
            return
