...
from graphgps.layer.graphrope import resolve_positional_method
...
class GPSLayer(nn.Module):
    ...
    def __init__(self, dim_h,
                 ...
                 graphrope_positional_active=True):
        super().__init__()
        ...
        if global_model_type == 'None':
            ...
            performer_method = resolve_positional_method(
                graphrope_method_override
                if graphrope_method_override is not None
                else getattr(graphrope_cfg, 'method', None) if graphrope_cfg is not None else None,
                getattr(graphrope_cfg, 'enable', False) if graphrope_cfg is not None else False,
            )
            if graphrope_positional_active and performer_method.startswith('aw'):
                self.self_attn = OfficialPerformerAWRoPE(
                    dim=dim_h,
                    heads=num_heads,
                    dropout=self.attn_dropout,
                    aw_cfg=getattr(graphrope_cfg, 'aw', None),
                    method=performer_method,
                )
                self.official_performer_aw = True
            else:
                self.self_attn = SelfAttention(
                    dim=dim_h, heads=num_heads,
                    dropout=self.attn_dropout, causal=False)
                self.official_performer_aw = False
            ...
            self.self_attn = GraphRoPE(
                ...
                positional_method=(graphrope_method_override
                                   if graphrope_method_override is not None
                                   else getattr(graphrope_cfg, 'method', None)),
                apply_positional=graphrope_positional_active,
                aw_cfg=getattr(graphrope_cfg, 'aw', None),
            )
        else:
    ...
    def forward(self, batch):
        ...
        if self.self_attn is not None:
            if self.global_model_type == 'GraphRoPE':
                ...
                # Preserve sparse topology for AW-RoPE.  WIRE continues to use
                # batch.t and all methods consume the same current hidden state.
                temp_batch = Batch(
                    x=h,
                    batch=batch.batch,
                    edge_index=batch.edge_index,
                ...
                if hasattr(batch, 't'):
                    temp_batch.t = batch.t
                if hasattr(batch, 'edge_weight'):
                    temp_batch.edge_weight = batch.edge_weight
                if hasattr(batch, 'pos'):
                    temp_batch.pos = batch.pos
                if hasattr(batch, '_aw_reverse_edge'):
                    temp_batch._aw_reverse_edge = batch._aw_reverse_edge
                if hasattr(batch, '_lr_aw_geometry_cache'):
                    temp_batch._lr_aw_geometry_cache = batch._lr_aw_geometry_cache
                if hasattr(batch, 'exact_holonomy_transport'):
                    temp_batch.exact_holonomy_transport = (
                        batch.exact_holonomy_transport
                    )
                if hasattr(batch, 'aw_static_edge_displacement'):
                    temp_batch.aw_static_edge_displacement = (
                        batch.aw_static_edge_displacement
                    )
                if hasattr(batch, '_exact_holonomy_geometry_cache'):
                    temp_batch._exact_holonomy_geometry_cache = (
                        batch._exact_holonomy_geometry_cache
                    )
                h_attn = self.self_attn(temp_batch)
                if hasattr(temp_batch, '_aw_reverse_edge'):
                    batch._aw_reverse_edge = temp_batch._aw_reverse_edge
            else:
