...
class LapPENodeEncoder(torch.nn.Module):
    ...
    def forward(self, batch):
        ...
        if self.expand_x:
            # Match the GIN continuous-input contract at the model boundary.
            # This is the shared node-input path used by both WIRE and AW, so
            # integer one-hot datasets such as PATTERN are treated identically
            # without changing the official dataset preprocessing.
            node_features = batch.x.to(dtype=self.linear_x.weight.dtype)
            h = self.linear_x(node_features)
        else:
            h = batch.x
